...
fn filter(ctx: &mut RequestContext) -> TiiResult<()> {
  ctx.add_header("test", "testo")?;
  ctx.add_header("test", "testo2")?;

  ctx.add_query_param("beep", "beep1");
  ctx.add_query_param("beep", "beep2");

  let r = ctx.set_query_param("mog", "cog");
  assert_eq!(r.len(), 2);
  assert_eq!("bog", r.first().unwrap().to_string());
  assert_eq!("log", r.get(1).unwrap().to_string());

  ctx.set_query_param("zog", "hog");

  let r = ctx.remove_query_params("rm");
  assert_eq!(r.len(), 2);
...
fn route(ctx: &RequestContext) -> Response {
  assert_eq!(Some("testo"), ctx.get_header("test"));
  assert_eq!(Some("xxxx"), ctx.get_query_param("bla"));

  let p = ctx.get_headers("test");
  assert_eq!(p.len(), 2);
  assert_eq!("testo", p.first().unwrap().to_string());
  assert_eq!("testo2", p.get(1).unwrap().to_string());

  let p = ctx.get_query_params("bla");
  assert_eq!(p.len(), 2);
  assert_eq!("xxxx", p.first().unwrap().to_string());
  assert_eq!("yyyyy", p.get(1).unwrap().to_string());

  let p = ctx.get_query_params("beep");
  assert_eq!(p.len(), 2);
  assert_eq!("beep1", p.first().unwrap().to_string());
  assert_eq!("beep2", p.get(1).unwrap().to_string());

  let p = ctx.get_query_params("mog");
  assert_eq!(p.len(), 1);
  assert_eq!("cog", p.first().unwrap().to_string());

  assert_eq!(ctx.get_query_param("nope"), None);
  assert_eq!(ctx.get_query_param("rm"), None);
  assert_eq!(ctx.get_query_params("rm").len(), 0);
