...
impl Address {
  ...
  pub fn new(addr: impl ToSocketAddrs) -> Result<Self, Box<dyn Error>> {
    let addr = addr.to_socket_addrs()?.next().ok_or("No socket address found")?;

    Ok(Self { origin_addr: addr.ip(), proxies: Vec::new(), port: addr.port() })
  }
  ...
  pub fn from_headers(headers: &Headers, addr: impl ToSocketAddrs) -> Result<Self, Box<dyn Error>> {
    if let Some(forwarded) = headers.get("X-Forwarded-For") {
      let mut proxies: Vec<IpAddr> =
        forwarded.split(',').filter_map(|s| IpAddr::from_str(s).ok()).collect();
      ...
      proxies.push(addr.to_socket_addrs()?.next().ok_or("No socket address found")?.ip());

      Ok(Self {
        origin_addr,
        proxies,
        port: addr.to_socket_addrs()?.next().ok_or("No socket address found")?.port(),
      })
    } else {
      Self::new(addr)
    }
