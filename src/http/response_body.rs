	...
	fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
		match self {
			...
			ResponseBodyInner::ChunkedGzipFile(_) => {
				f.write_str("ResponseBody::ChunkedGzipFile(...)")
			}
...
impl ResponseBody {
	...
	pub fn from_data_with_gzip_in_memory(data: impl AsRef<[u8]>) -> Self {
		...
		let mut encoder = crate::util::unwrap_ok(crate::util::new_gzip_encoder(
			Vec::with_capacity(data.len() + 128),
		));
		...
		match self.0 {
			...
			| ResponseBodyInner::ExternallyGzippedFile(mut file, size) => {
				...
				loop {
					...
					stream.write_all(
						io_buf.get_mut(..read).ok_or(io::Error::other("buffer overflow"))?,
					)?;
					written = written
						.checked_add(
							u64::try_from(read)
								.map_err(|_| io::Error::other("usize->u64 failed"))?,
						)
						.ok_or(io::Error::other("u64 overflow"))?;
				}
			}
			ResponseBodyInner::Stream(mut handler) => handler
				.take()
				.ok_or_else(|| TiiError::from_io_kind(io::ErrorKind::UnexpectedEof))?(
...
impl ResponseBodySink for GzipChunkedSink<'_> {
	...
	fn write_all(&self, buffer: &[u8]) -> io::Result<()> {
		trace_log!(
			"tii: Request {} GzipChunkedSink::write_all with {} bytes",
			self.0,
			buffer.len()
		);
		//Safety, this function will panic/abort if called after finish
