			...
			_ => {
				if !n.is_ascii_alphanumeric() {
					...
					return Err(TiiError::RequestHeadParsing(
						RequestHeadParsingError::InvalidPath(raw_path.to_string()),
					));
				}
			}
		}
...
fn parse_status_line(start_line_buf: &Vec<u8>) -> TiiResult<&str> {
	...
	error_log!(
		"parse_status_line: fatal error std::str::from_utf8 with buf failed utf8 validation even tho it succeeded ascii validation. buf={start_line_buf:?}"
	);
	...
	for n in raw_query.as_bytes().iter().copied() {
		if matching_percent != 0 {
			match n {
				...
				_ => {
					return Err(
						RequestHeadParsingError::InvalidQueryString(raw_query.to_string()).into()
					);
				}
			}
		}
		...
		match n {
			...
			b'&' => {
				...
				let key =
					urlencoding::decode(unwrap_ok(std::str::from_utf8(current_key.as_slice())))
						.map_err(|_| {
							RequestHeadParsingError::InvalidQueryString(raw_query.to_string())
						})?
						.replace('+', " ");

				let value =
					urlencoding::decode(unwrap_ok(std::str::from_utf8(current_value.as_slice())))
						.map_err(|_| {
							RequestHeadParsingError::InvalidQueryString(raw_query.to_string())
						})?
						.replace('+', " ");
...
impl RequestHead {
	...
	pub fn new(
			...
			for (idx, part) in path.split("/").enumerate() {
				if idx == 0 {
					if !part.is_empty() {
						...
						return Err(TiiError::RequestHeadParsing(
							RequestHeadParsingError::InvalidPath(path),
						));
					}
					continue;
				}
		...
		if version == HttpVersion::Http09 {
			...
			if !headers.is_empty() {
				return Err(TiiError::UserError(UserError::HeaderNotSupportedByHttpVersion(
					version,
				)));
			}
		...
		let status_line =
			format!("{} {}{} {}", method, path.as_str(), query_string.as_str(), version);
	...
	pub fn read(
		...
		let path = urlencoding::decode(raw_path)
			.map_err(|_| {
				TiiError::from(RequestHeadParsingError::InvalidPathUrlEncoding(
					raw_path.to_string(),
				))
			})?
			.replace('+', " ");
		...
		loop {
			...
			#[cfg(feature = "log")]
			{
				if log::max_level() == log::Level::Trace {
					if line.starts_with("Authorization:") {
						trace_log!(
							"tii: Request {id} next header line: Authorization: ***MASKED***"
						);
					} else {
			...
			let value =
				line_parts.next().ok_or(RequestHeadParsingError::HeaderValueMissing)?.trim();
	...
	pub fn set_header(&mut self, hdr: impl AsRef<str>, value: impl AsRef<str>) -> TiiResult<()> {
		...
		match &hdr.as_ref().into() {
			...
			HttpHeaderName::ContentType => {
				let mime = MimeType::parse(hdr_value).ok_or_else(|| {
					UserError::IllegalContentTypeHeaderValueSet(hdr_value.to_string())
				})?;
