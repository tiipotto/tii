...
use crate::{
	TypeSystem, TypeSystemError, UserError, debug_log, error_log, trace_log, util, warn_log,
};
...
impl RequestContext {
	...
	fn new_http10(
		...
		if let Some(content_length) = req.get_header(&HttpHeaderName::ContentLength) {
			let content_length: u64 = content_length.parse().map_err(|_| {
				TiiError::from(RequestHeadParsingError::InvalidContentLength(
	...
	fn new_http11(
		...
		match (
			...
			(None, None) => match content_length {
				None => {
					...
					if req.get_method().is_likely_to_have_request_body() {
						warn_log!(
							"tii: Request {id} did not sent Content-Length header but did request Connection: keep-alive. Assuming that it has no request body. The request method {} usually has a body, will force Connection: close to be safe.",
							req.get_method()
						);
				...
				Some(content_length) => {
					...
					Ok(RequestContext {
						...
						local_address,
						request: req,
						body: Some(RequestBody::new_with_content_length(
							stream.new_ref_read(),
							content_length,
						)),
						request_entity: None,
			...
			(None, Some("x-gzip")) | (None, Some("gzip")) => {
				trace_log!(
					"tii: Request {id} has gzip request body with length of uncompressed content"
				);
				let Some(content_length) = content_length else {
					error_log!(
						"tii: Request {id} not implemented no transfer encoding, Content-Encoding: gzip/x-gzip without Content-Length header"
					);
					return Err(TiiError::from(
						RequestHeadParsingError::ContentLengthHeaderMissing,
					));
				};

				let body = RequestBody::new_gzip_with_uncompressed_length(
					stream.new_ref_read(),
					content_length,
				)?;
			...
			(Some("gzip"), None) | (Some("x-gzip"), None) => {
				trace_log!(
					"tii: Request {id} has gzip request body with length of compressed content"
				);
				//gzip+Content-Length of zipped stuff
				let Some(content_length) = content_length else {
					error_log!(
						"tii: Request {id} not implemented Transfer-Encoding: gzip/x-gzip, no Content-Encoding without Content-Length header"
					);
					return Err(TiiError::from(
						RequestHeadParsingError::ContentLengthHeaderMissing,
					));
			...
			(other_encoding, other_transfer) => {
				match other_transfer {
					...
					Some(other) => {
						error_log!(
							"tii: Request {id} has unimplemented transfer encoding: {}",
							other
						);
						return Err(TiiError::from(
							RequestHeadParsingError::TransferEncodingNotSupported(
								other.to_string(),
							),
						));
					}
				...
				error_log!(
					"tii: Request {id} has unimplemented content encoding: {}",
					other_encoding
				);
	...
	pub fn cast_request_entity_mut<DST: Any + ?Sized + 'static, RET: Any + 'static>(
		...
		let caster =
			self.type_system.type_cast_wrapper_mut(Any::type_id(src), TypeId::of::<DST>())?;
