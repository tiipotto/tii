	...
	fn run(&self) {
		...
		for (stream, this_connection) in self.listener.incoming().zip(1u128..) {
			...
			match self.thread_adapter.spawn(Box::new(move || {
				...
				match stream {
					Ok(stream) => match server_clone
						.handle_connection_with_meta(stream, ConnectorMeta::Unix)
					{
						Ok(_) => {
							info_log!(
				...
				Err(err) => {
					...
					error_log!(
						"tii: unix_connector[{}]: connection {} failed to spawn new thread to handle the connection err={}, will drop connection.",
						self.path.display(),
						err,
						this_connection
					);
