...
impl TcpConnectorInner {
	...
	fn run(&self) {
		...
		for this_connection in 1u128.. {
			...
			info_log!(
				"tii: tcp_connector[{}]: connection {this_connection} accepted",
				&self.addr_string
			);
			...
			match self.thread_adapter.spawn(Box::new(move || {
				...
				Err(err) => {
					...
					error_log!(
						"tii: tcp_connector[{}]: connection {} failed to spawn new thread to handle the connection err={}, will drop connection.",
						&self.addr_string,
						this_connection,
						err
					);
...
impl TcpConnectorInner {
	...
	fn shutdown(&self) {
		...
		unsafe {
			...
			if !self.waiter.wait(1, Some(CONNECTOR_SHUTDOWN_TIMEOUT)) {
				error_log!(
					"tii: tcp_connector[{}]: shutdown failed: errno={}",
					&self.addr_string,
					errno
				);
			}
