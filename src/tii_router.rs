	...
	fn parse(path: impl AsRef<str>) -> TiiResult<Vec<PathPart>> {
		...
		loop {
			...
			if part == "*" {
				...
				if !path.is_empty() && path != "/" {
					return Err(
						InvalidPathError::MorePartsAfterWildcard(full_path.to_string()).into()
					);
				}
				return Ok(parts);
			}
			...
			if part.starts_with("{") && part.ends_with("}") {
				...
				if let Some((name, regex)) = variable.split_once(":") {
					let reg = Regex::new(regex).map_err(|e| match e {
						Error::Syntax(syntax) => InvalidPathError::RegexSyntaxError(
							full_path.to_string(),
							regex.to_string(),
							syntax,
						),
						Error::CompiledTooBig(limit) => InvalidPathError::RegexTooBig(
							full_path.to_string(),
							regex.to_string(),
							limit,
						),
		...
		match self {
			...
			PathPart::RegexTailVariable(var_name, regex) => {
				if regex.is_match(remaining) {
					...
					unwrap_some(variables.as_mut())
						.insert(var_name.to_string(), remaining.to_string());
					return true;
				}
				false
			}
...
impl DefaultRouter {
			...
			return match websocket_handshake(request) {
				...
				Ok(resp) => {
					...
					if resp.status_code != StatusCode::SwitchingProtocols {
						return Ok(RouterWebSocketServingResponse::HandledWithoutProtocolSwitch(
							resp,
						));
					}
	...
	fn serve_outer(&self, request: &mut RequestContext) -> TiiResult<Option<Response>> {
		...
		let mut resp =
			self.serve_inner(request).or_else(|e| self.call_error_handler(request, e))?;
		...
		if let Some(handler) = best_handler {
			...
			if request.get_request_entity().is_none() {
				if let Some(body) = request.request_body() {
					request.set_request_entity(
						handler.handler.parse_entity(
							request
								.request_head()
								.get_content_type()
