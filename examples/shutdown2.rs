...
fn main() -> HumptyResult<()> {
  _= colog::default_builder().format(|buf, record| {
  ...
  let connector = extras::TcpConnector::new("0.0.0.0:8080", humpty_server)?;

  let mut stream = TcpStream::connect_timeout(&SocketAddr::from_str("127.0.0.1:8080")?, Duration::from_secs(30))?;
