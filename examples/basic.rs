...
fn pre_routing(req: &mut RequestContext) -> io::Result<Option<Response>> {
  println!("pre_routing {:?}", req);
  Ok(None)
}

fn routing(req: &mut RequestContext) -> io::Result<Option<Response>> {
  println!("routing {:?}", req);
  Ok(None)
}

fn resp(req: &mut RequestContext, mut resp: Response) -> io::Result<Response> {
  println!("resp {:?}", req);
...
fn contact(_: &RequestContext) -> io::Result<Response> {
  Ok(Response::new(StatusCode::OK, "<html><body><h1>Contact</h1></body></html>"))
}

fn generic(request: &RequestContext) -> io::Result<Response> {
  let html = format!(
...
fn pong(request: &RequestContext) -> io::Result<Response> {
